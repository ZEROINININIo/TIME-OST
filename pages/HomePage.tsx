...
const HomePage: React.FC<HomePageProps> = ({ onNavigate, language, setLanguage }) => {
  ...
  return (
    // Replaced bg-halftone with transparent + backdrop blur for overlay effect
    <div className="flex flex-col min-h-full font-custom-02 relative bg-transparent">
      ...
      <div className="flex-1 flex flex-col p-4 md:p-8 pb-32 lg:pb-12">
          ...
          <div className="w-full max-w-7xl mx-auto mb-6 shrink-0">
            <Reveal className="w-full bg-ash-black/80 backdrop-blur-md border-2 border-ash-gray p-4 md:p-6 shadow-hard relative min-h-[160px] md:min-h-[180px] flex flex-col hover:border-ash-light transition-colors duration-500">
                ...
                <h2 className="text-xs font-bold text-ash-gray mb-4 flex items-center gap-2 uppercase border-b border-dashed border-ash-gray/30 pb-2">
                    <Terminal size={14} /> ROOT_CONSOLE // TL.1.14-Z24
                </h2>
          ...
          {/* Middle Section: Dual Entry Modules with New Visual Effects */}
          <div className="flex-1 w-full max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 md:gap-8">
                
                {/* Module 1: Main Story (Emerald Aurora Theme) */}
                <Reveal delay={200} className="flex-1">
                    <button 
                        onClick={() => onNavigate('reader')}
                        className="w-full h-full min-h-[240px] bg-ash-black/70 backdrop-blur-sm border-2 border-ash-gray hover:border-emerald-500/50 hover:shadow-[0_0_30px_rgba(16,185,129,0.3)] transition-all duration-500 group relative flex flex-col overflow-hidden text-left holo-card"
                    >
                        {/* Animated Aurora Background - ALWAYS VISIBLE now (low opacity) */}
                        <div className="absolute inset-0 bg-aurora opacity-10 group-hover:opacity-30 transition-opacity duration-700"></div>

                        {/* Animated Background Grid - Emerald Tint */}
                        <div className="absolute inset-0 bg-[linear-gradient(rgba(20,20,20,0)_1px,transparent_1px),linear-gradient(90deg,rgba(20,20,20,0)_1px,transparent_1px)] bg-[size:20px_20px] opacity-10 group-hover:opacity-100 transition-opacity duration-500 group-hover:bg-[linear-gradient(rgba(16,185,129,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(16,185,129,0.1)_1px,transparent_1px)]"></div>

                        {/* Header */}
                        <div className="w-full p-4 border-b-2 border-ash-gray bg-ash-dark/50 flex justify-between items-center group-hover:bg-emerald-950/50 group-hover:border-emerald-900/50 transition-colors duration-300 z-20 relative">
                        ...
                        <div className="p-6 md:p-10 flex-1 flex flex-col justify-center relative z-10">
                            <h3 className="text-3xl md:text-5xl font-black uppercase text-ash-light mb-4 group-hover:text-emerald-50 transition-colors duration-300 group-hover:translate-x-1 rgb-text" data-text={t.main_archive} style={{ textShadow: '0 0 0 rgba(0,0,0,0)' }}>
                                {t.main_archive}
                            </h3>
                ...
                {/* Module 2: Side Story (Cyan/Blue Aurora Theme) */}
                <Reveal delay={300} className="flex-1">
                    <button 
                        onClick={() => onNavigate('sidestories')}
                        className="w-full h-full min-h-[240px] bg-slate-950/70 backdrop-blur-sm border-2 border-slate-700 hover:border-cyan-400 hover:shadow-[0_0_30px_rgba(6,182,212,0.4)] transition-all duration-300 group relative flex flex-col overflow-hidden text-left holo-card"
                    >
                        {/* Animated Aurora Background (Blue variant) - ALWAYS VISIBLE */}
                        <div className="absolute inset-0 bg-aurora opacity-10 group-hover:opacity-30 transition-opacity duration-700" style={{ filter: 'hue-rotate(60deg)' }}></div>

                        {/* Header */}
                        <div className="w-full p-4 border-b-2 border-slate-700 bg-slate-900/50 flex justify-between items-center group-hover:bg-cyan-400 group-hover:text-black transition-colors z-20 relative">
                        ...
                        {/* Body */}
                        <div className="p-6 md:p-10 flex-1 flex flex-col justify-center relative z-10">
                            <h3 className="text-3xl md:text-5xl font-black uppercase text-cyan-100 mb-4 group-hover:text-cyan-300 group-hover:scale-105 transition-transform origin-left drop-shadow-[0_0_5px_rgba(34,211,238,0.5)] rgb-text" data-text={t.side_archive}>
                                {t.side_archive}
                            </h3>
      ...
      <div className="absolute bottom-20 lg:bottom-0 left-0 right-0 bg-ash-black/80 backdrop-blur-md border-t-2 border-ash-gray p-2 text-[10px] text-ash-gray font-custom-02 flex justify-between items-center select-none z-20">
          <div className="flex items-center gap-2 md:gap-4 px-2 overflow-hidden shrink-0">
               <div className="flex items-center gap-1 shrink-0">
                   ...
                   <span className="text-ash-light animate-pulse-glow">SECURE</span>
               </div>
